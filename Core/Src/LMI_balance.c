...
void LMIII(float theta, float theta_dot, float tracking_vd, float tracking_wd, float *remote_angle, float *delta_output, float v0, float distance_flag) {
	// New version of LMI written in 2022.8.4.
	// - Removed most of the magic numbers from the old version
	// - Seems to work in both remote control mode and tracking mode (not tested yet)
...
#endif
	// Remote output ranged between (+/-) 15 degrees then converted to radians
	float MX_TURN_LMT_DEG = 20.0f;
    float delta_remote = MX_TURN_LMT_DEG * PI / 180 * (float)(REMOTE_CENTER - RX1)/(float)(top - REMOTE_CENTER); // remote ranges from 152 ~ -152(turn up)

    // Add low pass filter to remote output
	float delta_remote_lp = delta_remote;
	static float delta_remote_old = 0.0f;
	delta_remote_lp = LowpassFilter(delta_remote_lp, delta_remote_old, 4.0f, dt);	//if tracking omega need lowpass
	delta_remote_old = delta_remote_lp;
...
#ifndef EKF
	tracking_vd = 2.0f;
	tracking_wd = tracking_vd*tan(delta_remote_lp*sin(epsilon))/b; // should be remote control wd, not 0
#endif
	...
	// Read delta_dot?
	int speed_refu16_tmp = 0;
	float speed_ref = 0.0f;
	uint8_t speed_state;
	float SPEED_VAL_UNIT = 0.11;
	speed_refu16_tmp = dxl_read_word(MOTOR_ID, P_PRESENT_SPEED_L);
	speed_state = dxl_get_result();
	if (speed_state == COMM_RXSUCCESS) {
		if (speed_refu16_tmp < 0x7ff && speed_refu16_tmp > 0x00) {
			speed_ref = (float) (speed_refu16_tmp & 0x3ff) * SPEED_VAL_UNIT * 2 * PI / 60; // Convert RPM to Rad/s
			// Check the 10th bit to determine the turning direction
			if (speed_refu16_tmp>>10 == 0x00){ // CW -> - (sign reversed here since MX is installed reversed)
				speed_ref = -speed_ref;
			}
		}
	}
	float speed_ref_lp = speed_ref;
	static float speed_ref_old = 0.0f;
	speed_ref_lp = LowpassFilter(speed_ref_lp, speed_ref_old, 2.0f, dt);	//if tracking omega need lowpass
	speed_ref_old = speed_ref_lp;
	...
	if (v0 > V_STD){
		// Generate the states x and x_d
//		float K[3] = {266.1011,  43.6233  , -17.3143};
//		float K[3] = {69.8768,   12.7423,   -2.9143};// YAY!!!     lambda=0.0
//		float K[3] = {87.9203,   16.7969,   -4.0722};// also works lambda=0.5
		float K[3] = {48.6900,    7.9318,   -1.5660};
//		float K[3] ={106.7959,   20.0679,   -7.2117}; //lambda =.5 XXX
//		float K[3] = {82.7697,   15.7489,   -3.1122};
		float x[3] = {theta + THETA_COM * PI / 180.0f, theta_dot, delta_ref};  // 2022.8.17 added THETA_COM to the first term
		float x_d[3] = {tracking_vd * tracking_vd * sinf(epsilon) / (-GRAVITY * b) * delta_d, 0, delta_d}; // here I think vd should be used instead of v0?
		float u_d = (tracking_vd / a) * delta_d;
		float u_bar = 0.0f;
		float u = 0.0f;
//		printf("%.3f,%.3f,%.3f\r\n", delta_d, delta_remote,u_d);
		// Compute x_bar
		for(int i = 0; i < 3; i ++){
			x[i] = x[i] - x_d[i];
		}
		...
		// Compute steering control u_bar. The computed u here is the angular velocity command, but the MX motor input is degrees (NEED CONVERSSION)
		for(int i = 0; i < 3; i ++){
			u -= K[i]/1.5*x[i];  // The MX motor is installed upside down (sign change? I think)
		}
		u_bar = u + u_d;
		...
		// According to the thesis, output (delta_cmd) = (u_bar - delta_dot) / (v/a + v_dot/v)
		// (For convenience, I don't consider v0_dot here)
		// Wrong, not used!!
//		float delta_dot = u_bar - (v0/a)*delta_ref;
		output = (u_bar - speed_ref_lp)/(v0/a);

		/* =================================
		 * ==== c2d delta/u transfer fcn ===
		 * ================================= */
		// Use Tustin method to transform the delta/u equation to discrete
		// Originally, the equation had a delta_dot (shouldn't be accessed directly because derivatives contain noise)
//		static float u_bar_old = 0.0f;
		static float output_old = 0.0f;
		float Ka = 2.54;
		float acc_term = Ka*(tracking_vd-v0);
		if(acc_term < -v0/a){acc_term = -v0/a;}

		// METHOD 1: Use s = 2/T(z-1)/(z+1) for transformation
//		float output_c2d = (u_bar + u_bar_old - output_old*(-2/sample_time+v0/a+acc_term))/(2/sample_time+v0/a+acc_term);
//		float output_c2d = (u_bar + u_bar_old - output_old)/(2/sample_time+v0/a+acc_term); // Works

		// METHOD 2: Use s = (z-1)/Tz for transformation
		// Works with the additional (a/v0) term
//		float output_c2d = (output_old/(v0/a)+sample_time*u_bar)/(1+v0/a*sample_time+acc_term*sample_time);
		float output_c2d = (output_old+sample_time*u_bar)/(1+v0/a*sample_time+acc_term*sample_time);
//		float output_c2d = (u_bar_old/(v0/a)+sample_time*u_bar)/(1+v0/a*sample_time+acc_term*sample_time);
		// Ref: output = (output_old/frequency + dt*input_u)/(1+frequency*dt);

		// METHOD 3: Use z = e^sT
//		float output_c2d = u_bar + expf((-v0/a-acc_term)*sample_time)*output_old;
//		printf("%.3f\r\n", expf((-v0/a-acc_term)*sample_time));

		// METHOD 4: Use simplest method
//		float output_c2d = u_bar_old*sample_time+(1-(v0/a+acc_term)*sample_time)*output_old;

//		u_bar_old = u_bar;
		output_old = output_c2d;

		float OUTPUT_MAX_RAD = 45 * PI / 180;  // ~0.349 ~0.523
		if (output_c2d > OUTPUT_MAX_RAD){ output_c2d = OUTPUT_MAX_RAD;}
		if (output_c2d < -OUTPUT_MAX_RAD){ output_c2d = -OUTPUT_MAX_RAD;}
		*delta_output = output_c2d;

//		/* =============================================
//		 * ==== c2d delta/u transfer fcn with v_dot ====
//		 * ============================================= */
//		static float v_old =0.0f;
//		float v_dot = (v0 - v_old)/sample_time;
//		v_old = v0;
//		if (v_dot < -v0/a){v_dot = -v0/a;}
//		static float u_bar_old = 0.0f;
//		static float output_old = 0.0f;
//		float output_c2d = (u_bar + u_bar_old - output_old*(-2/sample_time+v0/a+v_dot/v0))/(2/sample_time+v0/a+v_dot/v0);
//		u_bar_old = u_bar;
//		output_old = output_c2d;
...
//		*delta_output = output;
		...
		hexgoal = (uint16_t) ((MXDEC_CENTER - output_c2d * 180.0f / PI) / MXHEX2DEC);
//	    hexgoal = (uint16_t) ((MXDEC_CENTER - delta_d * 180.0f / PI) / MXHEX2DEC);
//		printf("%.3f,%.3f\r\n", tracking_vd, tracking_wd);
//	    printf("%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\r\n", u_bar, u, u_d, speed_ref, delta_ref,  delta_d, output, x[0],x[1],x[2]);
//	    printf("%.3f,%.3f,%.3f,%.3f\r\n", speed_ref,speed_ref_lp, delta_ref, delta_d);
//		printf("%.3f,%.3f,%.3f\r\n", output, output_lp,delta_d);
//		printf("%.3f,%.3f\r\n",output, output_c2d);
//		printf("%.3f,%.3f,%.3f\r\n", output_c2d, output_c2d/v0*a, u_bar);
//		printf("%.3f\r\n",theta_dot);
	#ifdef INIT_MX
